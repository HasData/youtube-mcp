...
const ENDPOINT = 'https://mcp.hasdata.com/api/mcp?apis=youtube';
const KEY = process.env.HASDATA_API_KEY;
const TIMEOUT_MS = 30_000;

const EXPECTED = {
    hasdata_youtube_search_getYoutubeSearchResults: 'q',
    hasdata_youtube_video_getYoutubeVideo: 'v',
    hasdata_youtube_channel_getYoutubeChannel: 'channelId',
    hasdata_youtube_transcript_getYoutubeTranscript: 'v',
};
...
test('apis=youtube exposes exactly four tools', live, async () => {
...
test('the key is accepted by HasData', live, async () => {
    const { raw } = await rpc('tools/call', {
        name: 'hasdata_youtube_video_getYoutubeVideo',
        arguments: { v: 'dQw4w9WgXcQ' },
    });
